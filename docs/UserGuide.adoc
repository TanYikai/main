...
Adds a remark to two persons in the address book simultaneously +
Format: `remark INDEX --INDEX r/REMARK`

[Note]
Can be used to keep track of the relationship status between people in the address book.
eg. What is the relationship between person with INDEX 1 and person with INDEX 3 in the address book.
Using the same command on the same INDEX will overwrite the previous remark.

****
* Add a remark to the two persons at both the specified `INDEX`. The index refers to the index number shown in the last person listing.
  The index *must be a positive integer* 1, 2, 3, ...
* Existing values will be updated to the input values.
* When editing remark, the existing remark of the person will be removed i.e adding of remark is not cumulative.
* You can remove the remark of both person by typing `r/` without specifying any remarks after it.
* To add individual remark, use the add command
* To edit individual remark, use the edit command
****

Examples:

* `remark 3 --2 r/Husband and wife`
* `remark 3 --2 r/`

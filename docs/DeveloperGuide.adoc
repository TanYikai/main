...
A more flexible add command that only requires the name to be specified at the time of its execution. The other fields such as phone, email and address need not be specfied at the time of add command execution. +
A separate parser for phone, email and address are used to accommodate this enhancement to isolate the interdependency between the modified enhanced add command with the other commands. +
...
// tag::RemarkCommand[]
=== Remark mechanism
The remark command takes in the user input through command line in the format remark INDEX --INDEX2 r/ REMARK, where INDEX and INDEX2 are 2 different indexes of persons in the addressbook and REMARK is the remark of the relationship of that person to another person. +
Eg. remark 1 --2 r/ Husband and Wife +

image::RemarkLogicDiagram.png[width="800"]

image::RemarkModelDiagram.png[width="800"]

==== Design Considerations

**Aspect:** Implementation of remark command +
**Alternative 1 (current choice):** Addition of remark to 2 person concurrently is a standalone command +
**Pros:** Reduced coupling of the concurrent remark command with the regular single person remark, and also enables easier extension of allowing the concurrent modification of persons beyond 2. +
**Cons:** More difficult to implement as new remark command classes and tests needs to be created. +
**Alternative 2:** Integrate the advanced concurrent remark command into edit command +
**Pros:** Easier to implement (implement spacing to create a new command class, just modify existing edit command and its parser). +
**Cons:** Increased coupling as the modification of existing edit command to accommodate this more advanced feature could break the current working edit command. +

Credits: Adapted from Yamgent https://github.com/se-edu/addressbook-level4/pull/599
// end::RemarkCommand[]
...
|`* *` |user |update the link:#confidence-estimate[confidence estimate] on a person's attributes |update confidence in pieces of information
...
|`* *` |user with many persons in the address book |hide details by default, and only show full details when I select a particular contact |have a less-cluttered address book
...
[discrete]
=== Use case: Add a named relationship between two persons

*MSS*

1.  User requests to list persons
2.  AddressBook shows a list of persons
3.  User requests to add an relationship between two specific persons
4.  AddressBook adds an relationship between those two persons
5.  User requests to show a list of relationships
6.  AddressBook shows a list of relationships
7.  User requests to add a name to that relationship
8.  AddressBook adds a name to that relationship
+
Use case ends.
...
* 7a. The index of relationship given to be added name on is invalid.
+
[none]
...
....
A percentage that represents how confident the user is in a particular attribute being accurate
....
...
....
A relationship between two people with a specific status (eg. lecturer-student)
....

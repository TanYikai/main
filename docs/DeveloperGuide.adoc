...
image::addCommandDemo.png[width="800"]

After: Only name is compulsory for a successful add command +

image::EnhancedAddDemo.png[width="800"]
...
An example is the `parsePhone` method which are also shared with `editCommand`, hence modifying the `parsePhone` method will cause a dependency issue for `editCommand` as well since they requires different parsing method after `addCommand` is enhanced. +
...
image::ParsePhone.png[width="800"]


It works by instead of returning an empty Optional field, example an empty `Phone` object which sets the internal phone strings to null, we used a substitute string "unspecified Phone" instead. +

This implementation ensured that the `Phone` object is not carrying a null string which can cause problems for other components but instead is carrying a predefined "unspecified Phone" string. +

This is applied across all other optional fields with the exception of name which is made compulsory. +

The initial compulsory checks for prefix is also changed from the original requiring all prefix to be present to the current only requiring name prefix to be compulsory. +

image::CompulsoryPrefixCheck.png[width="800"]
...
The Sort Command ensures encapsulation as the determining factor of which sort to use is only decided at the layer closest to where the data of the addressbook is. +

The `SortCommandParser` will parse the respective PREFIX that the user typed ( n/, p/, e/, a/, r/) and returns an enum of Option corresponding to the various sort types to `SortCommand`. +

image::SortCommandParser.png[width="800"]

The passed Option will then be deciphered in `UniquePersonList` which will instruct the `Person` object to activate the respective sort's comparator in each `Person` object. +

image::SortOptionDecide.png[width="800"]

Each `Person` will then use the passed Option to sort according to the desired comparable. +

image::PersonSortComparable.png[width="800"]
...
[discrete]
=== Use case: making remark using `add` command and `edit` command
...
1.  User requests to sort a list of persons by name, phone, email, address or remark
2.  AddressBook checks if input is valid and what type of sort to apply
3.  AddressBook sorts a list of persons alphanumerically by name, phone, email, address or remark
4.  AddressBook shows all sorted persons in the sorted list
+
Use case ends.
...
* 3a. The given prefix is invalid.
+
[none]
** 3a1. AddressBook shows an error message.
+
